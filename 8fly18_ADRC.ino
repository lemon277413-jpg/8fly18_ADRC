...
// PID控制器 (角度环、高度环、位置环仍使用PID)
PIDController pidController;

// ADRC角速度控制器 (替代内环角速度PID)
ADRC_Rate rollADRC;
ADRC_Rate pitchADRC;
ADRC_Rate yawADRC;
...
void setup() {
    ...
    // ADRC角速度环控制器 (替代内环PID)
    // b0 从 T-Motor AIR2216 KV880 + T1045 实测数据计算:
    //   Δ拉力/ΔPWM = 1.73g/PWM/电机 @ 50%油门
    //   Roll:  I_xx=0.13, 4电机/侧, 臂=0.30m → b0=18.0 °/s²/PWM
    //   Pitch: I_yy=0.56, 4电机/侧, 臂=0.30m → b0=4.2  °/s²/PWM
    //   Yaw:   I_zz=0.69, 8电机反扭矩   → b0=0.13 °/s²/PWM
    //   b0_roll/b0_pitch = 4.31 = I_yy/I_xx ✓ 物理自洽
...
void loop() {
    ...
    if(flag_200Hz) 
    {
        ...
        if (adrcActive) {
            rollOut  = rollADRC.update(targetRollRate, rollRate_200hz);
            pitchOut = pitchADRC.update(targetPitchRate, pitchRate_200hz);
            yawOut   = yawADRC.update(targetYawRate, yawRate_200hz);
        } else {
        ...
        rollOut = constrain(rollOut, -200.0f, 200.0f);
        pitchOut = constrain(pitchOut, -200.0f, 200.0f);
        ...
        switch (state) {
            ...
            case FlightState::INIT: {
                ...
                if (rc_ch3 < 1100) {
                    ...
                    if (rc_ch2 > 1150) {
                        ...
                        pidController.cleanPitchPIDData();
                        pidController.cleanYawPIDData();
                        // 复位ADRC的ESO状态（z1=0, z2=0），防止地面扰动估计残留
                        rollADRC.reset();
                        pitchADRC.reset();
                        yawADRC.reset();
                        // 清零目标角速率，防止首次ADRC更新时出现阶跃输入
                        targetRollRate = 0.0f;
            ...
            case FlightState::HOLD_POSITION: {
                ...
                if(state_counter == 0) {
                    ...
                    pidController.cleanRollPIDData();
                    pidController.cleanPitchPIDData();
                    rollADRC.reset();     // 清空ADRC角速度环扰动估计
                    pitchADRC.reset();    // 避免手动飞行历史扰动干扰定点模式
                    // 5. 串口打印锁存的目标值，方便调试
                    Serial.println("=====================================");
...
void debugPrint() {
    ...
    //   3-5:   rOut, pOut, yOut           — ADRC控制输出 (电机修正量)
    ...
    Serial.print(rollADRC.getOutput());  Serial.print(",");
    Serial.print(pitchADRC.getOutput()); Serial.print(",");
    Serial.print(yawADRC.getOutput());   Serial.print(",");

    // ADRC扰动估计 (核心: z2代表总扰动, 平稳飞行时应稳定在某个值)
    Serial.print(rollADRC.getZ2());  Serial.print(",");
    Serial.print(pitchADRC.getZ2()); Serial.print(",");
    Serial.print(yawADRC.getZ2());   Serial.print(",");
