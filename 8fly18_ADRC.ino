...
void setup() {
    ...
    // ADRC初始化: Ts=0.005, wo, B, KpOut, KpIn, KdIn, max_u
    //   Roll:  I_xx=0.13, 4电机/侧, 臂=0.30m → B=18.0 °/s²/PWM
    //   Pitch: I_yy=0.56, 4电机/侧, 臂=0.30m → B=4.2  °/s²/PWM
    //   Yaw:   I_zz=0.69, 8电机反扭矩   → B=2.0  °/s²/PWM
...
void calculateADRC() {
    // ADRC外环: 角度误差 → 角速度目标 (替代原angle PID)
    ADRC_OuterLoop(&adrcRoll, roll, targetRoll);
    ADRC_OuterLoop(&adrcPitch, pitch, targetPitch);
    ADRC_OuterLoop(&adrcYaw, yaw, targetYaw);
}
