...
extern uint8_t g_strength;
extern int16_t g_flow_vel_x;
extern int16_t g_flow_vel_y;
extern uint8_t g_flow_quality;
...
void micolink_read_data(int flow_x, int flow_y, int distance)
{
    flow_x = g_flow_vel_x;
    flow_y = g_flow_vel_y;
    distance = g_distance;
...
void micolink_decode(uint8_t data)
{
    ...
    switch(msg.msg_id)
    {
        case MICOLINK_MSG_ID_RANGE_SENSOR:
        {
            ...
            g_distance = payload.distance;       // 距离
            g_strength = payload.strength;       // 信号强度
            g_flow_vel_x = payload.flow_vel_x;   // 光流x轴速度
            g_flow_vel_y = payload.flow_vel_y;   // 光流y轴速度
            g_flow_quality = payload.flow_quality; // 光流质量
